...
def LeNet():
    ...
    pooling2 = layers.MaxPooling2D(
        pool_size = (1, 1),
        strides = (1, 1),
        padding = "valid",
        name = 'Max_pooling2')(conv2)
...
if __name__ == "__main__":
    ...
    #KFold_val = KFold(n_splits = 10, shuffle = True, random_state = 321)

    for train, test in KFold(n_splits = 10, shuffle = True, random_state = 321).split(x, t):
        ...
        model.summary()
        keras.utils.plot_model(model, to_file = 'CNN_architecture.pdf', show_shapes = True, show_layer_activations = True)
        ...
        print("%s: %f" % (model.metrics_names[0], results[0]))
        print("%s: %f" % (model.metrics_names[1], results[1]))
        cross_validation_loss_score.append(results[0])
        cross_validation_mae_score.append(results[1])
    print("k-fold test loss: {} (+/- {})".format(np.mean(cross_validation_loss_score), np.std(cross_validation_loss_score)))
    print("\n k-fold test mae: {} (+/- {})".format(np.mean(cross_validation_mae_score), np.std(cross_validation_mae_score)))
